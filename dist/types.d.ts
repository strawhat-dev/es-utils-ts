...
/** **plain** js objects. */
export type JsObject<value = JsValue> = {
    [key: string]: value;
};
/**
 ...
 */
export type KeyOf<T, Fallback = string, composite = Composite<T>> = StringKeyOf<composite> extends never ? Fallback : StringKeyOf<composite>;
/**
 * Like {@link KeyOf}, but for extracting a
 * union type from the **values** instead.
 */
export type ValueOf<T, Fallback = NonNullish, composite = Composite<T>> = KeyOf<T> extends keyof composite ? composite[KeyOf<T>] : Fallback;
/**
