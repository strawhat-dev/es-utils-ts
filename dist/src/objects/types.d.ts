import type { Composite, JsObject, KeyOf, KeyOfDeep, Maybe, Merge, Multi, Nullish, PartialDeep, ReadonlyDeep, Simplify, SimplifyDeep, Type, Union, ValueOf, ValueOfDeep, Writable } from '../types';
export type KeyDispatcher = <T extends object, Options extends KeyIterationOptions>(obj: Readonly<T>, options?: Options) => (Options['inherited'] extends true ? Union<KeyOf<T>> : KeyOf<T>)[];
...
export type MapCallback<T, Options extends ObjectOptions = {}> = Union<(key: Union<ResolvedKeys<T, Options>>, value: Union<ResolvedValues<T, Options>>) => Multi<[unknown, unknown]> | JsObject<unknown> | (false | Nullish)>;
