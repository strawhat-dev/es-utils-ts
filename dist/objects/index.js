 ...
 */
export const keysIn = (obj, predicate) => {
    const ret = [];
 ...
 */
export const keysOf = (obj, predicate) => {
 ...
 */
export const props = (obj, predicate) => {
    const set = new Set();
 ...
 */
export const pop = (obj, key = Object.keys(obj ?? {}).pop()) => {
    if (nullish(obj))
        return;
    const value = obj[key];
    delete obj[key];
...
export const map = (obj, ...args) => {
    ...
    for (const key of keys(obj, keyopts)) {
        ...
        const value = obj[key];
        if (deep && isObject(value))
            entry = [key, map(value, opts, callback)];
        else
...
const mapd = (property, value, options, resolver = (d) => (Array.isArray(d) ? d.includes(property) : !!d)) => ({
    value,
