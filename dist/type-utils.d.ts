import type { ConditionalExcept, ConditionalKeys, IsLiteral as LiteralCheck, LiteralToPrimitive, OmitIndexSignature, Simplify, StringKeyOf, UnionToIntersection } from 'type-fest';
...
export type Fn<ReturnValue extends Value = any, Params extends Value[] = any[]> = (...args: Params) => ReturnValue;
/**
...
/** **plain** js objects. */
export type JsObject<value = Value> = {
    [key: string]: value;
};
/**
 * More reliably extract a union of a given type's keys as strings.
 */
export type KeyOf<T, Resolved = Composite<T>> = Type<StringKeyOf<Resolved> extends never ? keyof T extends never ? string : keyof T : StringKeyOf<Resolved>>;
/**
 * Like {@link KeyOf}, but for extracting a
 * union type from the **values** instead.
 */
export type ValueOf<T, Resolved = Composite<T>> = KeyOf<T> extends keyof Resolved ? Resolved[KeyOf<T>] : NonNullish;
/**
 * Like {@link KeyOf}, but recursively extracts nested keys.
 */
export type KeyOfDeep<T, current = Composite<T>, nested extends keyof current = ConditionalKeys<current, JsObject<any>>> = Type<KeyOf<current> | (nested extends never ? never : KeyOfDeep<current[nested]>)>;
/**
 ...
 */
export type ValueOfDeep<T, current = Composite<T>, nested extends keyof current = ConditionalKeys<current, JsObject<any>>> = Type<current[KeysExcept<current, JsObject<any>>] | (nested extends never ? never : ValueOfDeep<current[nested]>)>;
/**
 ...
 */
export type Union<T> = Type<T | (IsLiteral<T> extends true ? LiteralToPrimitive<T> & NonNullish : Narrow<T>)>;
/**
 ...
 */
export type Composite<T, Intersection = UnionToIntersection<T>> = Type<T extends Function ? Extract<T, Fn> : T extends string | number ? ExtractLiteral<T> : OmitIndexSignature<Intersection>>;
/**
 ...
 * - `NonNullish` (fallback)
 */
export type Narrow<T> = Type<T extends primitive ? LiteralToPrimitive<T> : T extends Function ? Function : T extends readonly (infer Item)[] ? Narrow<Item | unknown>[] : T extends ReadonlySet<infer Item> ? Set<Narrow<Item>> : T extends ReadonlyMap<infer K, infer V> ? Map<Narrow<K>, Narrow<V>> : T extends Promise<infer Resolved> ? Promise<Narrow<Resolved>> : T extends JsObject<infer Values> ? JsObject<Narrow<Values>> : T extends object ? object : NonNullish>;
/**
 * @internal
